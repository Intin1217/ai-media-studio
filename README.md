...
**이미지 분석**
다중 이미지 드래그 앤 드롭 업로드를 지원하며, 업로드된 각 이미지에 동일한 모델로 객체 감지를 수행합니다. 결과는 바운딩 박스 오버레이가 적용된 갤러리 뷰로 확인할 수 있습니다. 로컬 Ollama Vision 모델(Qwen3-VL 등)을 연결하면 각 이미지에 대한 AI 상세 분석도 가능합니다. 이미지의 특정 영역을 드래그로 선택하여 해당 부분만 AI 상세 분석을 실행할 수도 있습니다. Ollama 분석 결과는 마크다운 형식으로 렌더링되어 구조화된 결과를 제공합니다.

**통계 대시보드**
IndexedDB(Dexie)에 누적된 감지 히스토리를 Recharts로 시각화합니다. 객체 빈도 바차트, 카테고리 파이차트, 감지 추이 라인차트, 성능 히스토리 차트를 제공하며, 감지 임계값 설정과 CSV/JSON 내보내기 기능도 포함되어 있습니다. 얼굴 분석 통계(성별 분포, 연령대 분포, 평균 체류/응시 시간)도 함께 시각화됩니다.

**실시간 얼굴 분석**
face-api.js 기반으로 웹캠에서 성별·나이를 추정하고 시선을 추적합니다. 체류 시간과 정면 응시 시간을 각각 측정하여 기록하며, 모든 처리는 브라우저 내에서 실행되어 영상이 서버로 전송되지 않습니다. 30프레임 이동평균 시간 평활화로 추정 안정성을 확보하고, 320×240 리사이즈 + 200ms throttle로 성능을 최적화했습니다. 유클리드 거리 기반 매칭으로 다중 얼굴을 동시 추적하며, 트래킹 종료 시 1건만 DB에 저장하여 동일 인물 중복을 방지합니다.

**PDF 실시간 번역**
로컬 Ollama 모델을 활용한 PDF 문서 번역 기능입니다. pdfjs-dist로 PDF를 파싱하고, 텍스트 블록을 추출하여 배치 번역합니다. 스트리밍 응답으로 번역 결과를 실시간으로 표시하며, 오류 발생 시 자동 재시도합니다. pretext 라이브러리로 번역된 텍스트의 레이아웃을 최적화하여 원본 레이아웃을 보존합니다. 원문/번역문 비교 사이드바와 호버 시 원문 확인 기능을 제공합니다.
...
| AI / Media           | TensorFlow.js COCO-SSD, MediaPipe EfficientDet, face-api.js (얼굴 감지/나이/성별/랜드마크), Ollama Vision (선택), WebRTC, Canvas API, pdfjs-dist, Tesseract.js, pretext |
...
| 실행 환경            | Docker (multi-stage build, docker-compose) / Node.js standalone                                                                                                         |
| CI                   | GitHub Actions (lint, type-check, test, build 자동 검증)                                                                                                                |
...
대시보드 사이드바는 3개 탭으로 구성됩니다.

- **일반** — 대시보드 탭 네비게이션 (실시간/이미지/통계/PDF/OCR)
- **브라우저 AI** — COCO-SSD/MediaPipe 모델 선택, 얼굴 분석 토글, 감지 임계값 설정 (실시간 감지 페이지에서는 모델 준비 상태만 표시되며, 모델 전환은 사이드바에서만 가능)
- **로컬 AI** — Ollama 연결 설정 및 Vision 모델 선택
...
Redux, Jotai, Recoil 등 여러 상태관리 라이브러리를 검토한 끝에 Zustand을 선택했습니다.

1. **최소한의 보일러플레이트**: Redux는 action, reducer, dispatch 패턴으로 코드량이 많지만, Zustand은 하나의 `create()` 호출로 스토어를 정의합니다. 실시간 감지처럼 상태 업데이트가 빈번한 곳에서 코드 복잡도를 낮출 수 있었습니다.
2. **React 외부에서 접근 가능**: 실시간 감지 루프는 `requestAnimationFrame` 콜백에서 돌아가므로 React 컴포넌트 바깥입니다. Zustand은 `getState()`/`setState()`로 React 트리 밖에서도 상태를 읽고 쓸 수 있어, Jotai나 Recoil처럼 React 컨텍스트에 의존하는 라이브러리보다 이 사용 사례에 적합합니다.
3. **selector 기반 리렌더링 최적화**: `useStore(s => s.field)` 패턴으로 필요한 상태만 구독하여 불필요한 리렌더링을 방지합니다. Context API의 전체 리렌더링 문제를 해결합니다.
4. **번들 사이즈**: Zustand은 약 1.1KB(gzip)로 Redux Toolkit(~12KB)이나 Recoil(~22KB)보다 훨씬 가볍습니다. 브라우저에서 AI 모델을 로드하는 프로젝트에서 번들 최적화는 중요합니다.

세션 간 유지가 필요한 영속 데이터(감지 히스토리)는 Dexie(IndexedDB)로 분리하여, 클라이언트 상태와 영속 데이터의 책임을 명확히 나눴습니다.
...
- [x] Phase 7 — 이미지 영역 선택 + 마크다운 렌더링 + 랜딩 온프레미스 전환
- [x] Phase 8 — 얼굴 분석 + PDF 번역 수정 + UX 개선
