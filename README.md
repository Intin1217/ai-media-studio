...
**실시간 얼굴 분석**
face-api.js 기반으로 웹캠에서 성별·나이를 추정하고 시선을 추적합니다. 체류 시간과 정면 응시 시간을 각각 측정하여 기록하며, 모든 처리는 브라우저 내에서 실행되어 영상이 서버로 전송되지 않습니다.
...
- **브라우저 AI** — COCO-SSD/MediaPipe 모델 선택, 얼굴 분석 토글, 감지 임계값 설정
